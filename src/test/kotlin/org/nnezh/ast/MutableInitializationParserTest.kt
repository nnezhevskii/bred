...
class MutableInitializationParserTest {
    ...
    private fun varKeyword() = Token.Keyword.Var(pos)
    ...
    ): Either<ASTError, MutableVariableInitializationASTNode> {
        val parser = MutableInitializationParser(expressionParser)
        return either { with(parser) { parse(TokensContext(tokens)) } }
    }

    private fun parseFromSource(src: String): Either<ASTError, MutableVariableInitializationASTNode> {
    ...
    fun `parses double literal with Double type`() {
        val result = parseFromSource("var Pi: Double = 3.1417")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    fun `parses boolean literal with Boolean type`() {
        val result = parseFromSource("var flag: Boolean = true")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    fun `parses string literal with String type`() {
        val result = parseFromSource("var msg: String = \"hi\"")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    fun `parses initialization with complex expression`() {
        val result = parseFromSource("var x: Int = a + b * 2")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    fun `parses initialization with variable expression`() {
        val result = parseFromSource("var y: Int = other")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    fun `parses initialization with spaces around punctuation`() {
        val result = parseFromSource("var x : Int = 42")
            .getOrElse { error("unexpected parse error: $it") }
    ...
    @Test
    fun `var without name fails`() {
        val result = parseInit(listOf(varKeyword(), colon(), identifier("Int"), assign(), eof()))
        assertTrue(result.isLeft())
    ...
    fun `keyword instead of variable name fails`() {
        val result = parseFromSource("var if : Int = 1")
        assertTrue(result.isLeft())
        assertTrue(result.leftOrNull()?.message?.contains("variable name") == true)
    }
    ...
    fun `literal instead of variable name fails`() {
        val result = parseFromSource("var 42 : Int = 1")
        assertTrue(result.isLeft())
        assertTrue(result.leftOrNull()?.message?.contains("variable name") == true)
    }
    ...
    fun `keyword instead of type fails`() {
        val result = parseFromSource("var x : if = 1")
        assertTrue(result.isLeft())
        assertTrue(result.leftOrNull()?.message?.contains("variable type") == true)
    }
    ...
    fun `literal instead of type fails`() {
        val result = parseFromSource("var x : 42 = 1")
        assertTrue(result.isLeft())
        assertTrue(result.leftOrNull()?.message?.contains("variable type") == true)
    }
    ...
    fun `unknown type fails`() {
        val result = parseFromSource("var x : Foo = 1")
        assertTrue(result.isLeft())
    ...
    fun `missing assign operator fails`() {
        val result = parseFromSource("var x : Int")
        assertTrue(result.isLeft())
    ...
    fun `missing expression fails`() {
        assertTrue(parseFromSource("var x : Int =").isLeft())
    }

    @Test
    fun `malformed expression fails`() {
        assertTrue(parseFromSource("var x : Int = 3 +").isLeft())
    }

    @Test
    fun `only var keyword fails`() {
        assertTrue(parseFromSource("var").isLeft())
    }

    @Test
    fun `var and name only fails`() {
        assertTrue(parseFromSource("var x").isLeft())
    }

    @Test
    fun `var name and colon only fails`() {
        assertTrue(parseFromSource("var x :").isLeft())
    }

    @Test
    fun `var name colon and type only fails`() {
        assertTrue(parseFromSource("var x : Int").isLeft())
    }
